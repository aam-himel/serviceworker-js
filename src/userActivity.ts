...
async function requestSignedUrl(): Promise<string | null> {
  try {
    const response = await fetch("give url here", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    console.error("Error during signed URL request:", (error as Error).message);
    return null;
  }
}
